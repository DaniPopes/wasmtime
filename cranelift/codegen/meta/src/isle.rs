...
pub fn get_isle_compilations(
    ...
    let src_isa_evm = codegen_crate_dir.join("src").join("isa").join("evm");
    #[cfg(feature = "pulley")]
    ...
    IsleCompilations {
        items: vec![
            ...
            // The EVM instruction selector.
            IsleCompilation {
                name: "evm".to_string(),
                output: gen_dir.join("isle_evm.rs"),
                inputs: vec![
                    prelude_isle.clone(),
                    prelude_lower_isle.clone(),
                    src_isa_evm.join("inst.isle"),
                    src_isa_evm.join("lower.isle"),
                ],
                untracked_inputs: vec![numerics_isle.clone(), clif_lower_isle.clone()],
            },
